import { app, BrowserWindow, ipcMain, WebContentsView, session, Menu, clipboard } from 'electron'
...
function getHeaderHeight(): number {
  return HEADER_HEIGHT + (findBarOpen && !contentFullscreen ? FIND_BAR_HEIGHT : 0)
}
...
function setupTabShortcuts(view: WebContentsView, tab: Tab): void {
  view.webContents.on('before-input-event', (event, input) => {
    ...
    if (ctrl && input.key === 't') {
      event.preventDefault(); const t = createTab(); switchTab(t.id); return
    }
    if (ctrl && input.key === 'w') {
      event.preventDefault(); closeTab(tab.id); return
    }
    ...
    if (ctrl && input.key === 'l') {
      event.preventDefault(); win.webContents.send('focus-url-bar'); return
    }
    ...
    if (ctrl && (input.key === '+' || input.key === '=')) {
      event.preventDefault(); setZoom(tab, tab.zoom + 0.1); return
    }
    if (ctrl && input.key === '-') {
      event.preventDefault(); setZoom(tab, tab.zoom - 0.1); return
    }
    if (ctrl && input.key === '0') {
      event.preventDefault(); setZoom(tab, 1.0); return
    }
    if (ctrl && input.key === 'f') {
      event.preventDefault()
      findBarOpen = true
    ...
    if (ctrl && shift && input.key === 'I') {
      event.preventDefault(); view.webContents.openDevTools(); return
    }
  })
}
...
function createTab(url = 'https://google.com'): Tab {
  ...
  const tab: Tab = {
    id: nextId++, view, title: 'New Tab', url,
    loading: true, canGoBack: false, canGoForward: false, zoom: 1.0,
  }
  tabs.push(tab)
...
function setupWinShortcuts(): void {
  win.webContents.on('before-input-event', (event, input) => {
    ...
    else if (ctrl && input.key === 'f') {
      event.preventDefault()
      findBarOpen = true
      updateBounds()
      win.webContents.send('find-open')
    }
    else if (!shift && ctrl && input.key === 'Tab') {
      event.preventDefault()
      const idx = tabs.findIndex(t => t.id === activeTabId)
...
function setupIPC(): void {
  ...
  ipcMain.on('find-search', (_, data: { text: string; forward: boolean }) => {
    if (!data.text) return
    getActive()?.view.webContents.findInPage(data.text, { forward: data.forward, findNext: false })
  })
  ipcMain.on('find-next', (_, data: { text: string; forward: boolean }) => {
    if (!data.text) return
    getActive()?.view.webContents.findInPage(data.text, { forward: data.forward, findNext: true })
  })
}
